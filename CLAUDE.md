...
**Muul** (मूळ, Sanskrit: *foundation*) is a minimal personal blog template built on Astro 6.
...
│   │   ├── PageHeader.astro    # Shared page header — title, optional meta slot, pagefind prop
│   │   ├── PostMeta.astro      # Date · read time · tags — shared meta primitive
│   │   ├── PostItem.astro      # Single post row — title + PostMeta (date only)
...
│   │   ├── Pagination.astro    # Prev/next for paginated routes
...
│   └── string.utils.ts     # ordinalSuffix — used by PostMeta for date formatting
...
### Add a font
1. Add the font config to the `fonts` array in `astro.config.mjs` (top-level, stable in Astro 6 — not under `experimental`)
2. Add the matching `<Font cssVariable="..." />` in `src/components/Head.astro`
3. Reference the CSS variable in `theme.css` or `base.css`
...
### Search
Search uses [Pagefind](https://pagefind.app) — a static index generated at build time.

- `@astro/pagefind` integration removed — incompatible with Astro 6
- `Search.astro` loads `/pagefind/pagefind-ui.js` via `is:inline` — bypasses Vite bundling entirely
- `PagefindUI` is instantiated from `window.PagefindUI` on `DOMContentLoaded`
- Index is generated by `pagefind --site dist` which runs automatically after `npm run build`
- Copy `dist/pagefind/` to `public/pagefind/` to make search available in dev and preview
- Search does not work in dev unless the index has been copied to `public/pagefind/`
- URL query sync via `processTerm` — `?q=` param set/cleared as user types
- CSP: `worker-src self blob:` required — Pagefind spawns a Web Worker via blob URL
...
7. **`astro.config.mjs` `fonts` array** — stable in Astro 6, no longer under `experimental`. Do not move back under `experimental`.
8. **`astro.config.mjs` `security.contentSecurityPolicy`** — CSP is active. Adding new inline scripts or external script sources requires updating directives. `is:inline` scripts are auto-nonced by Astro. `worker-src blob:` is required for Pagefind.
...
Version 0.6.0. Live at [muul.amitkul.in](https://muul.amitkul.in).
...
- Astro 6.0 — Vite 7, Zod 4, Fonts API stable, Node 22 required
- Component refactor — PostMeta, PostItem, PostList, Tag, TagList, Pagination
- Search fixed — Pagefind loaded via is:inline, bypasses Vite; titles indexed via PageHeader pagefind prop
- Content Security Policy — configured via security.contentSecurityPolicy
...
- Astro theme submission — unblocked, pending submission
- Astro theme submission — pending
